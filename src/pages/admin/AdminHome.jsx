...
import { FaQuestion, FaCheckCircle, FaClock, FaNewspaper } from 'react-icons/fa';

export default function AdminHome() {
  const { t } = useTranslation();
  const [stats, setStats] = useState({ total: 0, pending: 0, answered: 0, news: 0 });

  useEffect(() => {
    Promise.all([API.get('/questions'), API.get('/news/all')]).then(
      ([qRes, nRes]) => {
        const qs = qRes.data;
        setStats({
          total: qs.length,
